...
class SupertrendStrategy(IStrategy):
    # 'freqtrade hyperopt --strategy Supertrend --hyperopt-loss ShortTradeDurHyperOptLoss --timerange=20210101- --timeframe=1h --spaces all'
    ...
    # ROI table:
    minimal_roi = {"0": 0.1, "60": 0.05, "1440": 0.03, "2880": 0}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for multiplier in self.buy_m1.range:
            for period in self.buy_p1.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_1_buy_{multiplier}_{period}"))

        for multiplier in self.buy_m2.range:
            for period in self.buy_p2.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_2_buy_{multiplier}_{period}"))

        for multiplier in self.buy_m3.range:
            for period in self.buy_p3.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_3_buy_{multiplier}_{period}"))

        # Sell indicators
        for multiplier in self.sell_m1.range:
            for period in self.sell_p1.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_1_sell_{multiplier}_{period}"))

        for multiplier in self.sell_m2.range:
            for period in self.sell_p2.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_2_sell_{multiplier}_{period}"))

        for multiplier in self.sell_m3.range:
            for period in self.sell_p3.range:
                new_col = self.supertrend(dataframe, multiplier, period)["STX"]
                new_columns.append(pd.Series(new_col, name=f"supertrend_3_sell_{multiplier}_{period}"))
