...
from technical.util import resample_to_interval, resampled_merge, compute_interval
...
def gen_indicator(dataframe, indicator, timeframe, period):
    ...
    if len(splits) > 1:
        ...
        if timeframe != '5m':
            ...
            dataframe_sampled[indicator_name] = getattr(ta, indicator)(dataframe_sampled, period=period).iloc[:,
                                                col]
            dataframe = _resampled_merge(dataframe, dataframe_sampled, indicator_name)
            dataframe.ffill(inplace=True)
        else:
            dataframe[indicator_name] = getattr(ta, indicator)(dataframe, period=period).iloc[:, col]
    else:
        if timeframe != '5m':
            dataframe_sampled = resample_to_interval(dataframe, timeframe)
            dataframe_sampled[indicator_name] = getattr(ta, indicator)(dataframe_sampled, timeperiod=period)
            dataframe = _resampled_merge(dataframe, dataframe_sampled, indicator_name)
            dataframe.ffill(inplace=True)
        else:
...
def gen_condition(dataframe, left_indicator, left_timeframe, left_period,
                  _operator, right_indicator, right_timeframe, right_period, value):
    dataframe, left_indicator_name = gen_indicator(dataframe, left_indicator,
                                                   left_timeframe,
                                                   left_period)
    left_operand = dataframe[left_indicator_name]

    if right_indicator is not None:
        dataframe, right_indicator_name = gen_indicator(dataframe, right_indicator,
                                                        right_timeframe, right_period)
        right_operand = dataframe[right_indicator_name]

    else:  # handle in case not cross indicator
...
class Generate4CombineStrategy(IStrategy):
    ...
    cross_left_indicator = CategoricalParameter(cross_indicators, default=cross_indicators[0], space='buy',
                                                optimize=True)
    cross_left_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='buy', optimize=True)
    cross_left_period = CategoricalParameter(cross_time_periods, default=cross_time_periods[0], space='buy',
                                             optimize=True)
    cross_operator = CategoricalParameter(operators, default=">", space='buy', optimize=True)
    cross_right_indicator = CategoricalParameter(cross_indicators, default=cross_indicators[0], space='buy',
                                                 optimize=True)
    cross_right_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='buy', optimize=True)
    cross_right_period = CategoricalParameter(cross_time_periods, default=cross_time_periods[0], space='buy',
                                              optimize=True)
    ...
    compare_left_indicator = CategoricalParameter(compare_indicator, default=compare_indicator[0], space='buy',
                                                  optimize=True)
    compare_left_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='buy', optimize=True)
    compare_left_period = CategoricalParameter(compare_time_periods, default=compare_time_periods[0], space='buy',
                                               optimize=True)
    ...
    exit_cross_left_indicator = CategoricalParameter(cross_indicators, default=cross_indicators[0], space='sell',
                                                     optimize=True)
    exit_cross_left_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='sell', optimize=True)
    exit_cross_left_period = CategoricalParameter(time_periods, default=cross_time_periods[0], space='sell',
                                                  optimize=True)
    exit_cross_operator = CategoricalParameter(operators, default=">", space='sell', optimize=True)
    exit_cross_right_indicator = CategoricalParameter(cross_indicators, default=cross_indicators[0], space='sell',
                                                      optimize=True)
    exit_cross_right_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='sell', optimize=True)
    exit_cross_right_period = CategoricalParameter(time_periods, default=cross_time_periods[0], space='sell',
                                                   optimize=True)

    # exit_compare_left_indicator = CategoricalParameter(compare_indicator, default=indicators[0], space='buy',
    #                                                    optimize=True)
    # exit_compare_left_timeframe = CategoricalParameter(time_frames, default=time_frames[0], space='buy', optimize=True)
    # exit_compare_left_period = CategoricalParameter(compare_time_periods, default=time_periods[0], space='buy',
    #                                                 optimize=True)
    # exit_compare_operator = CategoricalParameter(operators, default=">", space='buy', optimize=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        try:
            dataframe, condition = gen_condition(dataframe=dataframe,
            ...
            dataframe, cross_condition2 = gen_condition(dataframe=dataframe,
            ...
            dataframe, compare_condition = gen_condition(dataframe=dataframe,
            ...
            dataframe, self_condition = gen_condition(dataframe=dataframe,
